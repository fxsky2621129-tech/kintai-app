...
(async()=>{
  ...
  try{
    ...
    await page.locator('#openLawGuideBtn').click();await page.locator('#lawYear').selectOption('2024');assert.equal(await page.locator('.law-card').count(),3);
    await page.locator('.law-confirm input').first().check();assert.match(await page.locator('#lawProgress').innerText(),/1 \/ 3/);
    await page.reload();await page.locator('#openLawGuideBtn').click();assert.equal(await page.locator('.law-confirm input').first().isChecked(),true);
    ...
    await page.locator('.law-confirm input').nth(1).click();assert.equal(await page.locator('.law-confirm input').nth(1).isChecked(),false);assert.match(await page.locator('#lawSaveStatus').innerText(),/保存できません/);
    await page.evaluate(()=>Storage.prototype.setItem=window.originalSet);
    const downloadEvent=page.waitForEvent('download');await page.locator('#closeLawGuideBtn').click();await page.locator('#backupBtn').click();const download=await downloadEvent;const backup=JSON.parse(fs.readFileSync(await download.path(),'utf8'));assert.deepEqual(backup.lawChecks,JSON.parse(saved));
    await page.evaluate(()=>{commitBackup(validateBackup({records:[],lawChecks:{}}));});assert.equal(await page.locator('.law-confirm input').first().isChecked(),false);
    ...
    await context.setOffline(true);await page.reload();await page.locator('#openLawGuideBtn').click();assert.equal(await page.locator('.law-card').count(),9);assert.equal(await page.locator('.law-confirm input').first().isChecked(),true);
    await page.locator('#closeLawGuideBtn').click();await page.locator('#returnPref').selectOption('東京都');await page.waitForFunction(()=>document.getElementById('returnCity').options.length>1);
    await page.locator('#openLawGuideBtn').click();await page.locator('.law-confirm input').nth(1).check();await page.reload();await page.locator('#openLawGuideBtn').click();assert.equal(await page.locator('.law-confirm input').nth(1).isChecked(),true);
    await page.goto(url+'gps-check.html');assert.match(await page.title(),/GPS/);await page.goto(url);
    await page.locator('#openLawGuideBtn').click();await page.screenshot({path:path.join(require('os').tmpdir(),'kintai-law-mobile.png')});
